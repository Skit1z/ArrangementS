...
leave_status = sa.Enum(
    "pending", "approved", "rejected", "withdrawn", "cancelled", name="leave_status"
)
...
swap_candidate_status = sa.Enum(
    "applied", "selected", "rejected", "expired", name="swap_candidate_status"
)
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_leave_requests_applicant_person_id", "leave_requests", ["applicant_person_id"]
    )
    ...
    op.create_index(
        "ix_swap_requests_requester_person_id", "swap_requests", ["requester_person_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "swap_request_id",
            sa.Uuid(),
            sa.ForeignKey("swap_requests.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "candidate_person_id",
