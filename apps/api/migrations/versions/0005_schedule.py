...
assignment_source = sa.Enum("auto", "manual", "swap", "replacement", "forced", name="assignment_source")
plan_assignment_status = sa.Enum("pending", "assigned", "vacant", "replaced", "cancelled", name="plan_assignment_status")
execution_status = sa.Enum("pending", "completed", "absent", "leave", "swapped", "task_cancelled", name="execution_status")
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("published_by", sa.Uuid(), sa.ForeignKey("users.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_table(
        "duty_slots",
        sa.Column("id", sa.Uuid(), primary_key=True),
        sa.Column("weekly_plan_id", sa.Uuid(), sa.ForeignKey("weekly_plans.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        "assignments",
        sa.Column("id", sa.Uuid(), primary_key=True),
        sa.Column("duty_slot_id", sa.Uuid(), sa.ForeignKey("duty_slots.id", ondelete="CASCADE"), nullable=False),
        sa.Column("person_id", sa.Uuid(), sa.ForeignKey("person_profiles.id", ondelete="RESTRICT"), nullable=True),
