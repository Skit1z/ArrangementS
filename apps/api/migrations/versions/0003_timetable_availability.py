...
building_type = postgresql.ENUM(
    "all", "main", "second", name="building_type", create_type=False
)
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("semester_id", sa.Uuid(), sa.ForeignKey("semesters.id", ondelete="RESTRICT"), nullable=False),
    ...
    op.create_table(
        "course_rules",
        sa.Column("id", sa.Uuid(), primary_key=True),
        sa.Column("timetable_upload_id", sa.Uuid(), sa.ForeignKey("timetable_uploads.id", ondelete="CASCADE"), nullable=False),
