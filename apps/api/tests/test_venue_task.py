...
def test_same_venue_overlap_rejected(db_session):
    ...
    task_service.create_task(
        ...
        venue_id=v.id,
        title="A",
        booking_start_at=datetime(2026, 3, 2, 9, 0),
        booking_end_at=datetime(2026, 3, 2, 11, 0),
    )
    db_session.commit()
    with pytest.raises(HTTPException) as ei:
        task_service.create_task(
            ...
            venue_id=v.id,
            title="B",
            booking_start_at=datetime(2026, 3, 2, 11, 15),
            booking_end_at=datetime(2026, 3, 2, 12, 0),
        )
...
def test_booking_end_before_start_rejected(db_session):
    ...
    with pytest.raises(HTTPException) as ei:
        task_service.create_task(
            ...
            venue_id=v.id,
            title="X",
            booking_start_at=datetime(2026, 3, 2, 12, 0),
            booking_end_at=datetime(2026, 3, 2, 11, 0),
        )
...
def test_required_people_min_1(db_session):
    ...
    with pytest.raises(HTTPException):
        task_service.create_task(
            ...
            required_people=0,
            booking_start_at=datetime(2026, 3, 2, 9, 0),
            booking_end_at=datetime(2026, 3, 2, 10, 0),
        )
...
def test_task_hours_preview_with_multiplier(db_session):
    ...
    task = task_service.create_task(
        ...
        venue_id=v.id,
        title="晚会",
        booking_start_at=datetime(2026, 3, 2, 18, 30),
        booking_end_at=datetime(2026, 3, 2, 19, 0),
    )
    db_session.commit()
