...
def _person(db, i=0, name="甲"):
    ...
    db.add(u); db.flush()
    p = PersonProfile(
        user_id=u.id, student_no=f"c{i}", class_name="一班", full_name=name,
...
def _slot_on(db, venue, day, start_hour=9, end_hour=11):
    ...
    return DutySlot(
        venue_id=venue.id, source_type=SlotSourceType.fixed_shift,
        slot_start_at=s, slot_end_at=e, required_people=1, credited_minutes=120,
    )
...
def test_constraint_outside_effective_range_ignored(db_session):
    ...
    db_session.add(PersonConstraint(
        person_id=p.id, constraint_type="forbid_venue",
        constraint_value={"venue_ids": [str(v.id)]}, is_hard=True, is_active=True,
        effective_start=date(2026, 1, 1), effective_end=date(2026, 1, 31),
    ))
    db_session.commit()
...
def test_constraint_open_ended_effective_start(db_session):
    ...
    db_session.add(PersonConstraint(
        person_id=p.id, constraint_type="forbid_venue",
        constraint_value={"venue_ids": [str(v.id)]}, is_hard=True, is_active=True,
        effective_start=date(2026, 2, 1), effective_end=None,
    ))
    db_session.commit()

    # 1 月（之前）可用
    assert eligibility.check_person_available_for_slot(db_session, p, _slot_on(db_session, v, date(2026, 1, 15))) is True
    # 3 月（之后）不可用
    assert eligibility.check_person_available_for_slot(db_session, p, _slot_on(db_session, v, date(2026, 3, 4))) is False
...
def _assignment_with(db, person, venue, day, hour=9, partner=None):
    ...
    plan = WeeklyPlan(week_start=day - timedelta(days=day.weekday()),
    ...
    slot = DutySlot(
        ...
        slot_start_at=s, slot_end_at=e, required_people=2 if partner else 1,
        credited_minutes=120, month_key=day.strftime("%Y-%m"), status=SlotStatus.filled,
    )
    ...
    if partner:
        a2 = Assignment(
            duty_slot_id=slot.id, person_id=partner.id, position_index=1,
            plan_status=PlanAssignmentStatus.assigned, execution_status=ExecutionStatus.pending,
            raw_minutes=120, weighted_minutes_before_round=Decimal(120),
            credited_minutes=120, balance_minutes=120,
        )
...
def test_swap_approve_rejects_when_partner_pair_forbidden(db_session):
    ...
    # 现有同班人配置「禁止与接替人搭档」
    db_session.add(PersonConstraint(
        person_id=existing_partner.id, constraint_type="no_pair_with",
        constraint_value={"person_ids": [str(receiver.id)]}, is_hard=True, is_active=True,
    ))
    db_session.commit()
    ...
    plan = WeeklyPlan(week_start=future - timedelta(days=future.weekday()),
    ...
    slot = DutySlot(
        ...
        slot_start_at=s, slot_end_at=e, required_people=2,
        credited_minutes=120, month_key=future.strftime("%Y-%m"), status=SlotStatus.filled,
    )
    db_session.add(slot); db_session.flush()
    requester_a = Assignment(
        duty_slot_id=slot.id, person_id=requester.id, position_index=0,
        plan_status=PlanAssignmentStatus.assigned, execution_status=ExecutionStatus.pending,
        raw_minutes=120, weighted_minutes_before_round=Decimal(120),
        credited_minutes=120, balance_minutes=120,
    )
    ...
    db_session.add(Assignment(
        duty_slot_id=slot.id, person_id=existing_partner.id, position_index=1,
        plan_status=PlanAssignmentStatus.assigned, execution_status=ExecutionStatus.pending,
        raw_minutes=120, weighted_minutes_before_round=Decimal(120),
        credited_minutes=120, balance_minutes=120,
    ))
    ...
    swap = swap_service.create_targeted(
        db_session, requester_person_id=requester.id,
        assignment_id=requester_a.id, target_person_id=receiver.id, reason="x",
    )
    swap_service.respond_target(db_session, target_person_id=receiver.id, swap_id=swap.id, accept=True)
    db_session.commit()

    # admin 终审：应当拒绝（no_pair_with 违反）
    with pytest.raises(HTTPException) as ei:
        swap_service.admin_approve(
            db_session, actor_id=None, swap_id=swap.id, selected_person_id=receiver.id,
        )
