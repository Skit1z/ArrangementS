...
def test_list_venue_tasks(client, seed_admin):
    ...
    v1 = client.post("/api/v1/venues", json={"name": "蓝厅", "code": "LT", "venue_type": "event_based"}, headers=h).json()
    v2 = client.post("/api/v1/venues", json={"name": "报告厅", "code": "TSG", "venue_type": "event_based"}, headers=h).json()

    # v1 三个任务：一个普通、一个取消；v2 一个普通
    t1 = client.post("/api/v1/venue-tasks", json={
        "venue_id": v1["id"], "title": "A",
        "booking_start_at": "2026-03-02T09:00:00", "booking_end_at": "2026-03-02T10:00:00",
    }, headers=h).json()
    t2 = client.post("/api/v1/venue-tasks", json={
        "venue_id": v1["id"], "title": "B",
        "booking_start_at": "2026-03-02T11:00:00", "booking_end_at": "2026-03-02T12:00:00",
    }, headers=h).json()
    client.post("/api/v1/venue-tasks", json={
        "venue_id": v1["id"], "title": "C",
        "booking_start_at": "2026-03-03T09:00:00", "booking_end_at": "2026-03-03T10:00:00",
    }, headers=h)
    client.post("/api/v1/venue-tasks", json={
        "venue_id": v2["id"], "title": "D",
        "booking_start_at": "2026-03-02T09:00:00", "booking_end_at": "2026-03-02T10:00:00",
    }, headers=h)
