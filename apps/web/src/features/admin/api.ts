...
export const adminApi = {
  ...
  timetables: {
    ...
    exportFree: async (week?: number) => {
      const res = await api.get("/timetables/export-free", {
      ...
      link.setAttribute(
        "download",
        `全员无课表_${week ? `第${week}周_` : ""}${new Date().toISOString().slice(0, 10)}.xlsx`
      );
...
export interface CurrentDutyItem {
  ...
  class_name: string;
  phone: string;
  previous_shift?: ShiftInfo | null;
  next_shift?: ShiftInfo | null;
}

export interface ShiftInfo {
  start_at: string;
  end_at: string;
  people: ShiftPersonItem[];
}
